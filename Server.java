          ...
          switch (request.getType()) {
            case RENEW:
              for (Battle item : battles) {
                if (item.player1.equals(request.getToPlayer())
                    || item.player2.equals(request.getToPlayer())) {
                ...
                if (threads.get(a).equals(this.getName())) {
                  ...
                  for (Battle battle : battles) {
                    if (battle.player2.equals(a) || battle.player1.equals(a)) {
                      ...
                      SocketUtil.send(players.get(a.equals(battle.player2)
                          ? battle.player1 : battle.player2), b);
                ...
                while ((line = reader.readLine()) != null) {
                  ...
                  if (info[0].equals(Username)) {
                    ...
                    if (info[1].equals(Password)) {
                      ...
                      for (Battle item : battles) {
                        if (item.player1.equals(request.getFromPlayer())
                            || item.player2.equals(request.getFromPlayer())) {
                          Message renew = new Message();
                          renew.setContent(item.state);
                          renew.setType(Message.Type.RENEW);
                          renew.setFromPlayer(item.player1.equals(request.getFromPlayer())
                              ? item.player1 : item.player2);
                          SocketUtil.send(socket, renew);
                          SocketUtil.send(players.get((item.player1.equals(request.getFromPlayer())
                              ? item.player2 : item.player1)), renew);
                        }
                      }
                    } else {
            ...
            case LOGOUT:
              ...
              break;
            case FIGHT:
              System.out.println(request.getFromPlayer() + " send a fight request to "
                  + request.getToPlayer());
              ...
              break;
            case FIGHT_SUCCESS:
              System.out.println("fight between " + request.getFromPlayer() + " and "
                  + request.getToPlayer() + " start successful");
            ...
            case FIGHT_FAILURE:
              ...
              break;
            case PLANT:
              System.out.println(request.getFromPlayer() + " plant in "
            ...
            case END:
              ...
              for (Battle i : battles) {
                if (i.player1.equals(request.getFromPlayer())
                    || i.player2.equals(request.getFromPlayer())) {
                  battles.remove(i);
                  break;
                }
              }
